...
browser.find_element_by_xpath(r'/html/body/div/div[2]/div[3]/div[2]/div[4]/div[2]/form/div[1]/input').send_keys(
    user)
browser.find_element_by_xpath(r'/html/body/div/div[2]/div[3]/div[2]/div[4]/div[2]/form/div[2]/input[1]').send_keys(
    password)
...
while 1:
    ...
    for room in rooms:
        ...
        # pprint(res)
        seat_id = res['seatStr']
        seat_id = re.findall('id="seat_(.*?)"', seat_id)
        if len(seat_id) > 0:
            flag = 1
            break
        else:
